...
-- EC NOTE: determined the unique actor_id 
UPDATE actor 
SET first_name = "HARPO"
WHERE actor_id = 172;

-- 4d. Perhaps we were too hasty in changing `GROUCHO` to `HARPO`. It turns out that `GROUCHO` was the correct name after all! In a single query, if the first name of the actor is currently `HARPO`, change it to `GROUCHO`. Otherwise, change the first name to `MUCHO GROUCHO`. BE CAREFUL NOT TO CHANGE THE FIRST NAME OF EVERY ACTOR TO `MUCHO GROUCHO`, HOWEVER! (Hint: update the record using a unique identifier.)
UPDATE actor 
SET first_name = "GROUCO"
...
WHERE payment.payment_date BETWEEN "2005-05-01 00:00:01" AND "2005-05-31 23:59:59"
GROUP BY staff.staff_id;

-- 6c. List each film and the number of actors who are listed for that film. Use tables `film_actor` and `film`. Use inner join.
SELECT  COUNT(film_actor.film_id), film.title
FROM film_actor
LEFT JOIN film
...
SELECT SUM(payment.amount) AS 'Total Amount', customer.first_name, customer.last_name
FROM payment 
JOIN customer
...
SELECT film.title, COUNT(rental.inventory_id) AS 'Num of Rentals'
FROM film 
JOIN inventory 
ON film.film_id = inventory.film_id
JOIN rental 
...
SELECT SUM(payment.amount) AS 'Total Amount', store.store_id
FROM payment 
JOIN customer 
